...
LOG_FILENAME = os.path.join(LOG_FOLDER, "draft_helper.log")
...
class Follower:
    ...
    def __handle_match_state_changed(self, blob):
        ...
        if "eventId" in game_room_config and "matchId" in game_room_config:
            self.current_match_event_id = (
                game_room_config["matchId"],
                game_room_config["eventId"],
            )
        ...
        if "finalMatchResult" in game_room_info:
            ...
            result = next(
                (r for r in reversed(results) if r.get("scope") == "MatchScope_Game"),
                None,
            )
            if result:
    ...
    def __handle_gre_to_client_message(self, message_blob):
        ...
        if message_blob["type"] == "GREMessageType_GameStateMessage":
            ...
            if (
                self.current_match_event_id is not None
                and self.current_match_event_id[0] == self.match_id
            ):
    ...
    def __handle_client_to_gre_message(self, payload):
        ...
        if payload["type"] == "ClientMessageType_SubmitDeckResp":
            ...
            deck = {
                ...
                "companion": deck_info.get(
                    "companionGRPId",
                    deck_info.get(
                        "companion", deck_info.get("deckMessageFieldFour", 0)
                    ),
                ),
    ...
    def __maybe_handle_game_over_stage(self, game_state_message):
        ...
        result = next(
            (r for r in reversed(results) if r.get("scope") == "MatchScope_Game"), None
        )
        if result:
            self.__send_game_end(
                won=self.seat_id == result["winningTeamId"],
    ...
    def __send_game_end(self, won, win_type, game_end_reason):
        ...
        response = self.__retry_post(
            f"{self.host}/{ENDPOINT_GAME_RESULT}", blob=game, use_gzip=True
        )
    ...
    def __handle_self_rank_info(self, json_obj):
        ...
        logger.info(
            f"Parsed rank info for {self.cur_user} as limited {self.cur_limited_level} and constructed {self.cur_constructed_level}"
        )
        response = self.__retry_post(
